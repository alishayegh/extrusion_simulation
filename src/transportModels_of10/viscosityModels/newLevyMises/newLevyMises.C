...
{
    defineTypeNameAndDebug(newLevyMises, 0);
    addToRunTimeSelectionTable(viscosityModel, newLevyMises, dictionary);
}
}
...
Foam::viscosityModels::newLevyMises::newLevyMises
(
    const word& name,
...
bool Foam::viscosityModels::newLevyMises::read
(
    const dictionary& viscosityProperties
)
{
    ...
    //newLevyMisesCoeffs = viscosityProperties.optionalSubDict
    //(
    //    typeName + "Coeffs"
    //);
...
void Foam::viscosityModels::newLevyMises::correct()
{
    tmp<volScalarField> tsigma = 2 * strainRate() * sqrt(3.0);
